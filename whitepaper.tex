...
\begin{abstract}
Australia's support services data --food relief, housing, health, legal aid, employment --is either proprietary or fragmented across disconnected government portals. No open, consolidated dataset exists. This paper maps the full data landscape, identifies the structural gap, and presents the Open Australian Services Archive (oa-sa): a working open-source pipeline that aggregates 24,497 services from 26 public sources into a single, openly licensed dataset. It then examines how the project could expand and what organisational sponsorship would make possible.
\end{abstract}
...
The Australian Charities and Not-for-profits Commission maintains a register of 65,000+ registered charities with bulk download available [6], including charity name, type, subtypes and address.

The ACNC register was briefly integrated into the oa-sa pipeline in April 2026 and then removed. A registered charity is not the same as an active service: the register contains no phone, email, opening hours or coordinates; descriptions are boilerplate strings generated from beneficiary tags (``Registered charity serving aged persons, children, families''); and thousands of records are umbrella-entity duplicates (e.g. a single diocesan trustee corporation registered many times, once per parish). Including the register inflated the dataset with 65,078 low-signal records and degraded overall data quality. The registry is more appropriately treated as background reference data than as a services directory.

The ACNC's \textit{Annual Information Statement (AIS) Programs} dataset is a more promising candidate for future integration: each row represents a program a charity runs, with program name, description, beneficiaries, and operating locations. This is service-shaped data rather than registry data, and may be incorporated in a later release.
...
The current dataset of 24,497 records represents what is achievable through public APIs and open data alone while holding a strict data-quality bar (84\% of records are classified \emph{complete}, carrying both a location and at least one contact channel). Several pathways exist for further expansion.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Additional OpenStreetMap tags} --libraries, clinics, pharmacies, and other facility types not currently extracted. Estimated 5,000+ additional records.
    \item \textbf{Shapefile conversion} --several government sources publish in Shapefile format only. Adding conversion support would unlock additional datasets.
\end{itemize}
...
\midrule
Data sources & 27 public API sources & + partnerships with state agencies and community organisations \\
\addlinespace
Coverage & 24,497 records & Potential to incorporate NHSD (390K), state directories, org directories \\
\addlinespace
